...
void SendNPCLoot(ChatHandler* handler, std::uint32_t id, std::string const& name, std::uint32_t quality, float chance, std::uint32_t minimum, std::uint32_t maximum, bool questRequired);
...
void SendItemRequirement(ChatHandler* handler, std::string const& kind, std::uint32_t id, std::string const& name, std::string const& required, std::string const& current, bool passed, std::string const& detail, std::uint32_t pathId, std::string const& pathName);
