...
void SendItemRequirement(ChatHandler* handler, std::string const& kind, std::uint32_t id, std::string const& name, std::string const& required, std::string const& current, bool passed, std::string const& detail, std::uint32_t pathId, std::string const& pathName) { handler->PSendSysMessage("AZERCORE_OPS|ITEM_REQUIREMENT|kind={}|id={}|name={}|required={}|current={}|passed={}|detail={}|pathid={}|pathname={}", Clean(kind), id, Clean(name), Clean(required), Clean(current), passed ? 1 : 0, Clean(detail), pathId, Clean(pathName)); }
