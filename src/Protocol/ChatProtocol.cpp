...
void SendEncounterHistoryEnd(ChatHandler* handler, std::uint32_t requestId, std::uint32_t count, std::uint32_t anomalies) { if (handler) handler->PSendSysMessage("AZERCORE_OPS|ENCOUNTER_HISTORY_END|request={}|count={}|anomalies={}", requestId, count, anomalies); }
