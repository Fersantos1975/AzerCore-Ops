...
void SendEncounterDiagnosticBegin(ChatHandler* handler, std::uint32_t requestId, std::uint32_t mapId, std::uint32_t instanceId, std::uint32_t difficulty, std::string const& mapName, std::string const& scriptName) { handler->PSendSysMessage("AZERCORE_OPS|ENCOUNTER_DIAG_BEGIN|request={}|map={}|instance={}|difficulty={}|name={}|script={}", requestId, mapId, instanceId, difficulty, Clean(mapName), Clean(scriptName)); }
void SendEncounterDiagnosticFinding(ChatHandler* handler, std::uint32_t requestId, std::string const& severity, std::string const& category, std::string const& subject, std::string const& expected, std::string const& actual, std::string const& detail, std::string const& recommendation) { handler->PSendSysMessage("AZERCORE_OPS|ENCOUNTER_DIAG_FINDING|request={}|severity={}|category={}|subject={}|expected={}|actual={}|detail={}|recommendation={}", requestId, Clean(severity), Clean(category), Clean(subject), Clean(expected), Clean(actual), Clean(detail), Clean(recommendation)); }
void SendEncounterDiagnosticRecovery(ChatHandler* handler, std::uint32_t requestId, std::string const& id, std::string const& title, std::string const& confidence, std::string const& evidence, std::string const& verificationCommand, std::string const& actionCommands, std::string const& recheckCommand, std::string const& expectedResult, std::string const& safety) { handler->PSendSysMessage("AZERCORE_OPS|ENCOUNTER_DIAG_RECOVERY|request={}|id={}|title={}|confidence={}|evidence={}|verify={}|actions={}|recheck={}|expected={}|safety={}", requestId, Clean(id), Clean(title), Clean(confidence), Clean(evidence), Clean(verificationCommand), Clean(actionCommands), Clean(recheckCommand), Clean(expectedResult), Clean(safety)); }
void SendEncounterDiagnosticEnd(ChatHandler* handler, std::uint32_t requestId, std::uint32_t passed, std::uint32_t warnings, std::uint32_t failures) { handler->PSendSysMessage("AZERCORE_OPS|ENCOUNTER_DIAG_END|request={}|passed={}|warnings={}|failures={}", requestId, passed, warnings, failures); }
void SendEncounterDiagnosticError(ChatHandler* handler, std::uint32_t requestId, std::string const& reason) { handler->PSendSysMessage("AZERCORE_OPS|ENCOUNTER_DIAG_ERROR|request={}|reason={}", requestId, Clean(reason)); }
void SendEncounterHistoryBegin(ChatHandler* handler, std::uint32_t requestId, std::uint32_t mapId, std::uint32_t instanceId, std::uint32_t difficulty, std::string const& mapName, std::uint32_t count) { if (handler) handler->PSendSysMessage("AZERCORE_OPS|ENCOUNTER_HISTORY_BEGIN|request={}|map={}|instance={}|difficulty={}|name={}|count={}", requestId, mapId, instanceId, difficulty, Clean(mapName), count); }
void SendEncounterHistoryEntry(ChatHandler* handler, std::uint32_t requestId, std::uint64_t sequence, std::uint64_t timestampMs, std::uint32_t encounterId, std::string const& name, std::uint32_t oldState, std::string const& oldName, std::uint32_t newState, std::string const& newName, std::string const& classification, std::string const& event, std::uint32_t attempt, std::uint32_t wipes, std::uint32_t kills, std::string const& detail) { if (handler) handler->PSendSysMessage("AZERCORE_OPS|ENCOUNTER_HISTORY_ENTRY|request={}|seq={}|time={}|id={}|name={}|old={}|oldname={}|new={}|newname={}|class={}|event={}|attempt={}|wipes={}|kills={}|detail={}", requestId, sequence, timestampMs, encounterId, Clean(name), oldState, Clean(oldName), newState, Clean(newName), Clean(classification), Clean(event), attempt, wipes, kills, Clean(detail)); }
void SendEncounterHistoryStats(ChatHandler* handler, std::uint32_t requestId, std::uint32_t encounterId, std::string const& name, std::uint32_t attempts, std::uint32_t wipes, std::uint32_t kills) { if (handler) handler->PSendSysMessage("AZERCORE_OPS|ENCOUNTER_HISTORY_STATS|request={}|id={}|name={}|attempts={}|wipes={}|kills={}", requestId, encounterId, Clean(name), attempts, wipes, kills); }
void SendMechanicEvent(ChatHandler* handler, std::uint32_t requestId, std::uint64_t sequence, std::uint64_t timestampMs, std::uint64_t elapsedMs, std::uint32_t encounterId, std::string const& type, std::uint32_t creatureEntry, std::uint64_t creatureGuid, std::string const& creatureName, std::string const& mechanicId, std::string const& mechanicName, std::string const& phase, std::string const& detail) { if (handler) handler->PSendSysMessage("AZERCORE_OPS|MECHANIC_EVENT|request={}|seq={}|time={}|elapsed={}|encounter={}|event={}|entry={}|guid={}|creature={}|mechanic={}|mechanicname={}|phase={}|detail={}", requestId, sequence, timestampMs, elapsedMs, encounterId, Clean(type), creatureEntry, creatureGuid, Clean(creatureName), Clean(mechanicId), Clean(mechanicName), Clean(phase), Clean(detail)); }
void SendEncounterHistoryEnd(ChatHandler* handler, std::uint32_t requestId, std::uint32_t count, std::uint32_t anomalies, std::uint32_t mechanicEvents) { if (handler) handler->PSendSysMessage("AZERCORE_OPS|ENCOUNTER_HISTORY_END|request={}|count={}|anomalies={}|mechanics={}", requestId, count, anomalies, mechanicEvents); }
void SendEncounterHistoryError(ChatHandler* handler, std::uint32_t requestId, std::string const& reason) { if (handler) handler->PSendSysMessage("AZERCORE_OPS|ENCOUNTER_HISTORY_ERROR|request={}|reason={}", requestId, Clean(reason)); }
