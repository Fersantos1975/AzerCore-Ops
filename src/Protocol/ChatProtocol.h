...
void SendEncounterDiagnosticBegin(ChatHandler* handler, std::uint32_t requestId, std::uint32_t mapId, std::uint32_t instanceId, std::uint32_t difficulty, std::string const& mapName, std::string const& scriptName);
void SendEncounterDiagnosticFinding(ChatHandler* handler, std::uint32_t requestId, std::string const& severity, std::string const& category, std::string const& subject, std::string const& expected, std::string const& actual, std::string const& detail, std::string const& recommendation);
...
void SendEncounterDiagnosticEnd(ChatHandler* handler, std::uint32_t requestId, std::uint32_t passed, std::uint32_t warnings, std::uint32_t failures);
void SendEncounterDiagnosticError(ChatHandler* handler, std::uint32_t requestId, std::string const& reason);
void SendEncounterHistoryBegin(ChatHandler* handler, std::uint32_t requestId, std::uint32_t mapId, std::uint32_t instanceId, std::uint32_t difficulty, std::string const& mapName, std::uint32_t count);
void SendEncounterHistoryEntry(ChatHandler* handler, std::uint32_t requestId, std::uint64_t sequence, std::uint64_t timestampMs, std::uint32_t encounterId, std::string const& name, std::uint32_t oldState, std::string const& oldName, std::uint32_t newState, std::string const& newName, std::string const& classification, std::string const& event, std::uint32_t attempt, std::uint32_t wipes, std::uint32_t kills, std::string const& detail);
void SendEncounterHistoryStats(ChatHandler* handler, std::uint32_t requestId, std::uint32_t encounterId, std::string const& name, std::uint32_t attempts, std::uint32_t wipes, std::uint32_t kills);
void SendMechanicEvent(ChatHandler* handler, std::uint32_t requestId, std::uint64_t sequence, std::uint64_t timestampMs, std::uint64_t elapsedMs, std::uint32_t encounterId, std::string const& type, std::uint32_t creatureEntry, std::uint64_t creatureGuid, std::string const& creatureName, std::string const& mechanicId, std::string const& mechanicName, std::string const& phase, std::string const& detail);
void SendEncounterHistoryEnd(ChatHandler* handler, std::uint32_t requestId, std::uint32_t count, std::uint32_t anomalies, std::uint32_t mechanicEvents);
void SendEncounterHistoryError(ChatHandler* handler, std::uint32_t requestId, std::string const& reason);
